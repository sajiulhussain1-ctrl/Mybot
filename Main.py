...
def send_qr_code(call):
    ...
    payment_info = (
        ...
        "1️⃣ Upar dikh rahe QR Code ko kisi bhi App (PhonePe, Paytm, GooglePay) से scan karein.\n"
    ...
    try:
        ...
        print("API Error, falling back to message:", e)
...
if __name__ == "__main__":
    # Background Flask server for 24/7 uptime
    Thread(target=run_flask, daemon=True).start()
    print("Flask server started successfully.")
    
    # Auto-restart loop to prevent bot from going offline
    while True:
        try:
